...
simulate_data <- function(N, seed) {
  ...
  b1 = log(2)
  b2 = 1.5
  betas1 <- c(b1,-b1,0,0,b1,-b1,b2,-b2,0,0,b2,-b2)
  betas2 <- c(0,0,b1,-b1,b1,-b1,0,0,b2,-b2,b2,-b2)
  betas1Q<-c(b1,-b1,0,0,b1,-b1)
  betas2Q<-c(0,0,b1,-b1,b1,-b1)
  # create censoring times (unrelated to survival time)
  cens <- rexp(N,0.01)
  # create the two hazards for the events 
  lambda1<-rate*exp(
                      betas1Q[1]*x1^2+ betas1Q[2]*x2^2+ betas1Q[3]*x3^2+
                      betas1Q[4]*x4^2+ betas1Q[5]*x5^2+ betas1Q[6]*x6^2)
  
  lambda2<-rate*exp(betas2Q[1]*x1^2+
                      betas2Q[2]*x2^2+ betas2Q[3]*x3^2+
                      betas2Q[4]*x4^2+ betas2Q[5]*x5^2+ betas2Q[6]*x6^2)
  ...
  for (i  in 1:5) {
    ...
    IBSF1<- sapply(idxsplit, function(x) {
      ...
      ###grow a forest
      FG.fit <- FGR(Hist(dt, e)~x1+x2+x3+x4+x5+x6,data=train_data,cause=1)
      forestC       <-rfsrc(Surv(dt, e)~x1+x2+x3+x4+x5+x6, data=train_data,
                            ntree = 200,nsplit = 2,mtry =4,cause=c(1,1))
...
n_sims <- 5 # At each sample size we repeat n_sims times
...
write.csv(results,'Quadratic_results.csv')
